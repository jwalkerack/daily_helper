...
def load_questionnaire(path: Path = QUESTIONNAIRE_PATH) -> dict:
...
def render_question(question: dict):
    ...
    if question_type == "short_text":
        return st.text_input(display_label, key=question_id, placeholder=placeholder, help=help_text)

    if question_type == "email":
        return st.text_input(display_label, key=question_id, placeholder=placeholder, help=help_text)

    if question_type == "long_text":
        return st.text_area(display_label, key=question_id, placeholder=placeholder, help=help_text)

    if question_type == "single_select":
        options = question.get("options", [])
        return st.selectbox(display_label, options=[""] + options, key=question_id, help=help_text)

    if question_type == "multi_select":
        options = question.get("options", [])
        return st.multiselect(display_label, options=options, key=question_id, help=help_text)
    ...
    if question_type == "checkbox":
        return st.checkbox(display_label, key=question_id, help=help_text)

    if question_type == "yes_no":
        return st.radio(display_label, options=["", "Yes", "No"], key=question_id, horizontal=True, help=help_text)
